...
*/
$MONGO["servers"][$i]["mongo_name"] = "%%%MONGO_SERVER%%%";//mongo server name
//$MONGO["servers"][$i]["mongo_sock"] = "/var/run/mongo.sock";//mongo socket path (instead of host and port)
$MONGO["servers"][$i]["mongo_host"] = "%%%MONGO_SERVER%%%";//mongo host
...
$MONGO["servers"][$i]["control_auth"] = %%%ROCKMONGO_AUTH%%%;//enable control users, works only if mongo_auth=false
$MONGO["servers"][$i]["control_users"]["%%%ROCKMONGO_USER%%%"] = "%%%ROCKMONGO_PASSWORD%%%";//one of control users ["USERNAME"]=PASSWORD, works only if mongo_auth=false
